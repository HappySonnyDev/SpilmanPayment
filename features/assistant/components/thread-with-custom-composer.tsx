...
} from "@/features/assistant/components/attachment";
import { MarkdownText } from "@/features/assistant/components/markdown-text";
import { ToolFallback } from "@/features/assistant/components/tool-fallback";
import { TooltipIconButton } from "@/features/assistant/components/tooltip-icon-button";
import { Button } from "@/components/ui/button";
import { cn } from "@/lib/shared/utils";
import { LazyMotion, MotionConfig, domAnimation } from "motion/react";
import * as m from "motion/react-m";
import { ChunkAwareComposer } from "@/features/assistant/components/chunk-aware-composer";
...
const AuthAwareSuggestions: React.FC<{ onAuthRequired: () => void; setPendingMessage: (msg: string) => void }> = ({
  onAuthRequired,
  setPendingMessage
}) => {
  const { user } = useAuth();
  ...
  const handleSuggestionClick = (action: string) => {
    ...
    // For authenticated users, directly send the message via runtime
        ...
        >
          <ThreadPrimitive.Viewport className="aui-thread-viewport relative flex flex-1 flex-col overflow-x-auto overflow-y-scroll px-4">
            ...
            <ThreadPrimitive.If empty={false}>
              <div className="aui-thread-viewport-spacer min-h-32 grow" />
            </ThreadPrimitive.If>
            <div className="aui-composer-wrapper sticky bottom-0 mx-auto flex w-full max-w-[var(--thread-max-width)] flex-col gap-4 overflow-visible rounded-t-3xl bg-background pb-4 md:pb-6">
              <ThreadScrollToBottom />
              <ThreadPrimitive.Empty>
                <AuthAwareSuggestions 
...
const EditComposer: FC = () => {
  return (
    <div className="aui-edit-composer-wrapper mx-auto flex w-full max-w-[var(--thread-max-width)] flex-col gap-4 px-2 first:mt-4">
      <div className="aui-edit-composer-root ml-auto flex w-full max-w-7/8 flex-col rounded-xl bg-muted">
        <textarea
...
const BranchPicker: FC<BranchPickerPrimitive.Root.Props> = ({
  ...
  return (
    <BranchPickerPrimitive.Root
      ...
      className={cn(
        "aui-branch-picker-root inline-flex items-center text-xs",
        className,
      )}
      {...rest}
    >
